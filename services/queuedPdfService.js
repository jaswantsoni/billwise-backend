...
class QueuedPdfService {
  ...
  async processQueue() {
    ...
    try {
      log(`[PDF Queue] 🚀 Starting job ${jobId} (${this.activeJobs}/${this.maxConcurrency} active, ${this.queue.length} queued)`);
      
      const pdfBuffer = await this.callGotenberg(job.html, job.options);
      
      const duration = Date.now() - startTime;
      log(`[PDF Queue] ✅ Job ${jobId} completed in ${duration}ms - PDF size: ${(pdfBuffer.length / 1024).toFixed(1)} KB`);
      
      job.resolve(pdfBuffer);
    } catch (error) {
      const duration = Date.now() - startTime;
      logError(`[PDF Queue] ❌ Job ${jobId} failed after ${duration}ms:`, error.message);
      
      if (job.retries < this.maxRetries) {
        job.retries++;
        const retryDelay = 1000 * job.retries;
        log(`[PDF Queue] 🔄 Retrying job ${jobId} in ${retryDelay}ms (attempt ${job.retries}/${this.maxRetries})`);
        setTimeout(() => {
          this.queue.unshift(job);
          this.processQueue();
        }, retryDelay);
      } else {
        logError(`[PDF Queue] 💀 Job ${jobId} failed permanently after ${this.maxRetries} attempts`);
  ...
  async callGotenberg(html, options = {}) {
    const startTime = Date.now();
    log(`[PDF Service] Starting PDF generation - HTML size: ${html.length} chars`);
    ...
    try {
      ...
      if (options.css) {
        ...
        log(`[PDF Service] CSS provided - size: ${options.css.length} chars`);
      }
      ...
      log(`[PDF Service] Gotenberg options:`, gotenbergOptions);
      ...
      log(`[PDF Service] Making request to: ${this.gotenbergUrl}/forms/chromium/convert/html`);
      ...
      log(`[PDF Service] ✅ PDF generated successfully in ${duration}ms - Size: ${(pdfSize / 1024).toFixed(1)} KB`);

      return Buffer.from(response.data);
    } catch (error) {
      const duration = Date.now() - startTime;
      logError(`[PDF Service] ❌ Generation failed after ${duration}ms:`, error.message);
      
      if (error.code === 'ECONNREFUSED') {
        logError(`[PDF Service] 🔌 Connection refused to ${this.gotenbergUrl}`);
        throw new Error('Gotenberg service is not available. Please check if the service is running.');
      } else if (error.code === 'ETIMEDOUT') {
        logError(`[PDF Service] ⏰ Request timed out after ${this.timeout}ms`);
        throw new Error('PDF generation timed out. The document might be too complex.');
      } else if (error.response) {
        logError(`[PDF Service] 🚫 Gotenberg error: ${error.response.status} - ${error.response.statusText}`);
        if (error.response.data) {
          logError(`[PDF Service] Response data:`, error.response.data.toString().substring(0, 500));
        }
        throw new Error(`Gotenberg error: ${error.response.status} - ${error.response.statusText}`);
      } else {
        logError(`[PDF Service] 💥 Unexpected error:`, error);
  ...
  async healthCheck() {
    try {
      ...
      logError('[PDF Service] Health check failed:', error.message);
      return false;
    }
  }
  ...
  clearQueue() {
    ...
    this.queue = [];
    log(`[PDF Queue] Cleared ${clearedJobs} jobs from queue`);
    return clearedJobs;
  }
  ...
  async wakeUpService() {
    try {
      log('[PDF Service] Attempting to wake up Gotenberg...');
      const isHealthy = await this.healthCheck();
      if (isHealthy) {
        log('[PDF Service] Gotenberg is already awake');
        return true;
      }
      log('[PDF Service] Waiting for Gotenberg to wake up...');
      await new Promise(resolve => setTimeout(resolve, 15000));
      
      return await this.healthCheck();
    } catch (error) {
      logError('[PDF Service] Failed to wake up service:', error.message);
      return false;
    }
  }
}
